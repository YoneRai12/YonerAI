...
Status: public-safe dependency-security gate for the Local LLM Conversation MVP, provider-compatibility follow-up, and `ora-ui` retirement cleanup.
...
- `ora-ui` alerts before retirement cleanup: 71
...
The old `ora-ui` surface is now retired instead of dependency-remediated in place. Removing its source tree and lockfile is the smallest safe way to avoid building on an unwanted UI and to reduce false priority in Dependabot.
...
|---|---|---|---|---|---|---|---|
| #12 | critical | `next` | npm | `ora-ui/package-lock.json` | retired `ora-ui` | Remove retired surface from active public repo; close related Dependabot PRs after default branch updates. | RETIRE_SURFACE |
| grouped | high | `next` | npm | `ora-ui/package-lock.json` | retired `ora-ui` | Remove retired surface from active public repo. | RETIRE_SURFACE |
| grouped | high | `electron` | npm | `ora-ui/package-lock.json` | retired `ora-ui` | Do not remediate by preserving the old Electron/PWA surface. | RETIRE_SURFACE |
| grouped | high | `minimatch`, `fast-uri`, `lodash`, `lodash-es`, `serialize-javascript`, `rollup`, `preact`, `picomatch`, `flatted`, `@babel/plugin-transform-modules-systemjs` | npm | `ora-ui/package-lock.json` | retired `ora-ui` | Remove manifest from active public repo. | RETIRE_SURFACE |
...
- `ora-ui` was retired from the active public repo instead of remediated in place.
