...
| #558 review/check rollup | Intake runs triggered by review/comment activity could remain failed after the finding was classified, leaving the PR merge state unstable despite a later passing intake label run. | process/security | Fixed in PR #558 by making unreviewed activity mark `needs-intake` with a warning instead of leaving stale failed check runs after classification. |
