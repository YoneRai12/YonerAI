...
- Do not put secrets, private runtime truth, live route maps, break-glass details, raw chain-of-thought, usernames, hostnames, or local absolute paths in public artifacts.
...
- Do not deploy, create production signing keys, create production trust stores, claim persistent memory completion, claim Google login completion, claim Discord restored, claim full hybrid completion, start v7.8, or claim production readiness.
...
- A security/runtime PR is current only when the affected path exists on current main and the replacement has tests or a clear non-reproducibility proof.
