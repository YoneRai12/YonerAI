...
| Conversation Session Scaffold | `core/src/ora_core/sessions.py`, `core/src/ora_core/api/routes/public_messages.py`, `tests/test_public_conversation_session_mvp.py` | Groups public messages by `session_id` / `conversation_id` and returns turn metadata with unique generated ids and bounded in-memory state. | new session scaffold tests | `RELEASE_READY` for non-persistent scaffold | same-experience session metadata | In-memory only; process restart clears it. | Use as base for later memory/identity contract, not as memory. |
