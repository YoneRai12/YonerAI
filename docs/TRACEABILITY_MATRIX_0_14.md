...
- latest narrow validation outcome:
  - `16 passed, 50 warnings`
- repaired rows:
  - `POST /v1/messages`: auth precedence row = cleared
  - `GET /v1/runs/{run_id}/events`: meta exposure row = cleared
- remaining strict concern:
  - `GET /v1/runs/{run_id}/events`: `reasoning_summary safe exposure` remains `partial`
- warnings are currently classified as non-current-batch blockers
