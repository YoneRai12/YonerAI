# Traceability Matrix 0.9

Status:

- planning gate artifact
- docs / tests / code 対応表
- execution gate candidate refresh applied
...
| `GET /v1/runs/{run_id}/events` | `reasoning_summary` は safe summary のみを露出し raw chain-of-thought を契約化しない | `docs/contracts/sse-run-events.md`, `docs/REASONING_SUMMARY_OWNER_REVIEW_0_1.md`, `docs/EXECUTION_GATE_CANDIDATE_0_1.md` | `MISSING TEST` | `core/src/ora_core/api/routes/runs.py` (pass-through serializer only); inspected `core/src/ora_core/engine/simple_worker.py`, `core/src/ora_core/brain/process.py`, and `core/src/ora_core/mcp/runner.py` emit other event families only; no `reasoning_summary` emit / write site found in inspected `core/src/ora_core`; producer / sanitizer owner unresolved | gap |
...
- non-gating open gaps for a future Track A candidate
