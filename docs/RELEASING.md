# Releasing ORA (Date-Based Versions)

ORA releases are now **date-based** and stored in `VERSION` using:
...
5. Tag and push:
   - `git tag -a v<VERSION> -m "ORA <VERSION>"`
   - `git push --tags`

## CI / GitHub Release
On tag push (`v*`), GitHub Actions will:
- Verify `VERSION` matches the tag (supports SemVer and DateVer).
- Create `ORA-<VERSION>.zip` via `git archive`.
- Publish a GitHub Release named `ORA <VERSION>` using `docs/releases/<VERSION>.md` as the body.
