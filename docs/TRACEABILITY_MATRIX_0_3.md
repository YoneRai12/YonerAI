...
| file normalization | local file は run contract を越える前に file reference へ正規化される。exact `fileref:` wire shape は未固定 | `docs/contracts/file-download-boundary.md`, `docs/DISTRIBUTION_NODE_MVP.md` | `MISSING TEST` | `core/src/ora_core/distribution/files.py::normalize_tool_result_for_run` | gap |
...
以下はまだ `MISSING DOC` または traceability gap を埋めるために残っている durable docs。
