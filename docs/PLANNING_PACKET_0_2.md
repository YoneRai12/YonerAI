...
- Internal Run API v0.1 はこの phase では fixed anchor として扱う
...
- freeze 対象は `src/cogs/ora.py` と reject 済み事項
- current lane は `Distribution Node MVP` のみ
- Internal Run API v0.1 は fixed anchor
...
- SSE terminal は `final` か `error` のどちらか 1 回だけ
...
- PDF は参照用であり、実装上の正本は repo 内 markdown
- public artifacts は private internals を直接 import しない
- cross-repo interaction は contract 経由だけ
- whitepaper / planning docs は live operational truth の完全代替ではない
...
- approvals surface の durable docs 正本ファイル名
- files contract の test evidence 追加有無

重要:

- `UNCONFIRMED` は current truth の否定ではない
- `UNCONFIRMED` を code change の理由に使わない
...
- current branch は safe pause point だが、freeze scope ではない
...
- auth precedence / SSE terminal rule / file-ref-only / files boundary が docs 上で一意に読める
- docs / tests / code の traceability matrix が v0.2 として埋まっている
...
- durable contract docs が planning artifact から参照可能になっている
