...
def _print_memory_pretty(report: dict[str, Any], *, lang: str = "en", color: ColorMode = "auto") -> None:
    if report.get("operation") == "status":
        if lang == "ja":
            ...
            print(render_report("YonerAI 記憶境界", (CliSection("状態", rows),), color=color))
            return
        rows = (
    ...
    if report.get("operation") == "list":
        ...
        print(render_report(title, (CliSection(section, rows),), color=color))
        return
    rows = (
...
def _build_auth_status_report_for_cli(args: argparse.Namespace) -> dict[str, Any]:
    return build_google_auth_status(_load_config_for_policy(args))


def _build_google_login_report_for_cli(args: argparse.Namespace) -> dict[str, Any]:
    if not args.dry_run:
        raise CliError("auth google login requires --dry-run in the public repo.", exit_code=2)
    return build_google_login_dry_run(_load_config_for_policy(args))


def _build_privacy_status_report_for_cli(args: argparse.Namespace) -> dict[str, Any]:
...
def _print_auth_pretty(report: dict[str, Any], *, lang: str = "ja", color: ColorMode = "auto") -> None:
    ...
    flow_rows = (
        ...
        CliRow("embedded_webview_allowed", flow.get("embedded_webview_allowed"), "fail" if flow.get("embedded_webview_allowed") else "ok"),
    ...
    storage_rows = (
        ...
        CliRow("plain_text_token_storage_allowed", storage.get("plain_text_token_storage_allowed"), "fail" if storage.get("plain_text_token_storage_allowed") else "ok"),
    ...
    sections = (CliSection(flow_title, flow_rows), CliSection(storage_title, storage_rows), CliSection(boundary_title, non_action_rows))
    if error:
        sections = (*sections, CliSection("Error", (CliRow(str(error.get("code")), error.get("message"), "warn"),)))
    print(render_report(title, sections, color=color))


def _print_privacy_pretty(report: dict[str, Any], *, lang: str = "ja", color: ColorMode = "auto") -> None:
    ...
    if lang == "ja":
        ...
        boundary_title = "Non-actions"
    sharing_rows = (
        CliRow("requested", data_sharing.get("openai_shared_traffic_requested"), "warn" if data_sharing.get("openai_shared_traffic_requested") else "ok"),
        CliRow("enabled", data_sharing.get("openai_shared_traffic_enabled"), "fail" if data_sharing.get("openai_shared_traffic_enabled") else "ok"),
        CliRow("requires_explicit_opt_in", data_sharing.get("requires_explicit_opt_in"), "ok" if data_sharing.get("requires_explicit_opt_in") else "fail"),
        CliRow("runtime_supported", data_sharing.get("runtime_supported"), "warn" if data_sharing.get("runtime_supported") else "ok"),
    )
    exclusion_rows = (
    ...
    ledger_rows = (
        CliRow("shared_traffic_flag_recorded", ledger.get("shared_traffic_flag_recorded"), "ok" if ledger.get("shared_traffic_flag_recorded") else "fail"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    auth_status = auth_subcommands.add_parser("status", help="Show auth readiness and disabled production-login boundaries.")
    ...
    auth_google_login = auth_google_subcommands.add_parser("login", help="Dry-run Google OAuth login contract. No live OAuth is started.")
    ...
    auth_google_login.add_argument("--lang", choices=LANG_CHOICES, default="ja", help="Pretty output language. Default: ja.")
    auth_google_login.add_argument("--color", choices=COLOR_CHOICES, default="auto", help="Pretty output color mode. Default: auto.")
    ...
    privacy_status = privacy_subcommands.add_parser("status", help="Show data sharing, private-content exclusion, and auth privacy state.")
    ...
    memory_forget.add_argument("--lang", choices=LANG_CHOICES, default="ja", help="Pretty output language. Default: ja.")
    ...
    memory_delete.add_argument("--lang", choices=LANG_CHOICES, default="ja", help="Pretty output language. Default: ja.")
...
def run(argv: list[str] | None = None) -> int:
    ...
    if args.command == "auth":
        if args.auth_command == "status":
            report = _build_auth_status_report_for_cli(args)
        elif args.auth_command == "google" and args.auth_google_command == "login":
            report = _build_google_login_report_for_cli(args)
        else:
            raise CliError("unknown auth command", exit_code=2)
        if args.json:
            _print_json(report)
        else:
            _print_auth_pretty(report, lang=args.lang, color=args.color)
        return 0 if report["ok"] else 1
    if args.command == "privacy" and args.privacy_command == "status":
        report = _build_privacy_status_report_for_cli(args)
        if args.json:
            _print_json(report)
        else:
    ...
    if args.command == "memory":
        ...
        if args.json:
            ...
            _print_memory_pretty(report, lang=getattr(args, "lang", "en"), color=args.color)
        return 0 if report["ok"] else 1
