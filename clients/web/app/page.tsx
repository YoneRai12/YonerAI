...
export default function Home() {
  ...
  const handleSend = async (event?: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorBody = toPublicMessageError(body);
        throw new Error(errorBody.message || errorBody.error || `Core API returned ${res.status}.`);
      }

      if (!isPublicMessageResponse(body)) {
        throw new Error("Core API returned a malformed mock response.");
      }
