...
@pytest.mark.asyncio
async def test_distribution_node_files_are_refs_only_and_downloadable(distribution_app, monkeypatch, tmp_path) -> None:
    session_local = distribution_app["session_local"]
    core_token = "distribution-file-test-token"
    monkeypatch.setenv("ORA_CORE_API_TOKEN", core_token)
    ...
    with TestClient(distribution_app["app"]) as client:
        ticket = client.post(
            ...
            json={},
            headers={
                **_auth_headers("web", "dist-user-files", display_name="tester"),
                "X-ORA-Core-Token": core_token,
            },
        )
...
@pytest.mark.asyncio
async def test_distribution_node_rejects_absolute_artifact_paths(distribution_app, tmp_path) -> None:
    session_local = distribution_app["session_local"]
    absolute_ref = str((tmp_path / "absolute-artifact.txt").resolve())
