...
def test_default_update_manifest_keeps_stable_users_on_stable_channel() -> None:
    ...
    assert stable_default.name == "manifest.v0.8.0.json"
    assert stable_report["latest_manifest_version"] == "0.8.0"
    assert stable_report["update_available"] is True
    assert stable_from_alpha_report["latest_manifest_version"] == "0.8.0"
...
def test_v070_site_and_release_docs_explain_bridge_boundaries() -> None:
    ...
    assert "Latest stable: `v0.8.0`." in install_page
    assert "YonerAI CLI Local Runtime v0.8.0" in install_page
