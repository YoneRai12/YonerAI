...
[Japanese README](README_JP.md) | [Current phase](docs/CURRENT_PHASE_CONTEXT.md) | [Contracts](docs/contracts) | [Latest checkpoint](docs/releases/v2026.5.21.4-implementation-guardrail-compression-checkpoint.md)
...
The current public checkpoint stream uses verified dates with same-day suffixes. `v2026.5.21.4` records the implementation-first guardrail compression pass: a `/say` security/runtime patch, Discord contract acceptance tests, a three-mode capability harness, `src/cogs/ora.py` extraction planning, and ORA pure-helper contract tests. It is a checkpoint note and GitHub Release alignment target, not a production release.
