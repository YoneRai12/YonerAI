...
echo Killing tracked ORA tunnel processes (pid files)...
set "ROOT_DIR=%~dp0.."
powershell -NoProfile -Command "$td = Join-Path '%ROOT_DIR%' 'state\\tunnels'; if (Test-Path $td) { Get-ChildItem $td -Filter *.pid | ForEach-Object { try { $pid = [int](Get-Content $_.FullName -Raw).Trim(); if ($pid -gt 0) { Stop-Process -Id $pid -Force -ErrorAction SilentlyContinue } } catch {}; Remove-Item $_.FullName -Force -ErrorAction SilentlyContinue } }"
