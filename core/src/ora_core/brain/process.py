...
class MainProcess:
    ...
    async def run(self):
        ...
        try:
            ...
            required_passes = self._band2_pass_count(effective_route)
            pass_timeout_sec = self._band2_pass_timeout_sec()
            ...
            for turn in range(max_turns):
                pass_index = min(current_pass, required_passes)
                if (time.monotonic() - started_at) >= float(time_budget_seconds):
                    ...
                    break
                logger.info(
                    f"Run {self.run_id} Turn {turn+1}/Pass {pass_index}: Generating response (Pref: {llm_pref})..."
                )
                await self._emit_progress("memory", pass_index, ["context"])
                await self._emit_progress("compose", pass_index, ["draft"])
                ...
                try:
                    ...
                    await event_manager.emit(
                        ...
                        "error",
                        {
                            ...
                            "user_safe_message": "Core timed out before completing the run.",
                        },
                    )
                    return
                ...
                if not tool_calls:
                    ...
                    completed_passes = max(completed_passes, pass_index)
                    if pass_index < required_passes:
                        current_pass = pass_index + 1
                        await self._emit_progress("plan", current_pass, ["revise"])
                        continue
                    break
                else:
            ...
            await self._persist_effective_route(effective_route)
            terminal_pass = max(1, completed_passes or min(current_pass, required_passes))
            await self._emit_progress("deliver", terminal_pass, [])
            await event_manager.emit(
                self.run_id,
                "final",
                {
